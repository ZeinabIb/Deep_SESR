...
class Deep_SESR_GraphOP:
    def __init__(self, frozen_model_path):
        ...
        self.sess = tf.compat.v1.Session(graph=self.gen_graph)
    ...
    def predict(self, frame):
        ...
        output_dict = self.sess.run(
            self.output_dict, feed_dict={self.inp_im_tensor: np.expand_dims(frame, 0)}
        )
...
if num_test == 0:
    ...
    print("Time taken: {0} sec at {1} fps".format(Ttime, 1.0 / Mtime))
